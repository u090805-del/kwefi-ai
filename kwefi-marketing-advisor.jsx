...
export default function KwefiAdvisor() {
  ...
  const sendMessage = async (text) => {
    ...
    const categoryContext = selectedCategory
      ? `[Focus area: ${selectedCategory.en}] `
      : "";
    ...
    try {
      ...
      const reply = data.content?.map((b) => b.text).join("") || "عذراً، حدث خطأ.";
      setMessages([...newMessages, { role: "assistant", content: reply }]);
    } catch {
      setMessages([...newMessages, { role: "assistant", content: "⚠️ حدث خطأ في الاتصال. حاول مرة أخرى." }]);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;900&family=DM+Sans:wght@400;500;600&family=Noto+Naskh+Arabic:wght@400;600;700&display=swap');
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 20px 120px" }}>
          ...
          <div className="fade-up" style={{ textAlign: "center", marginBottom: 32, paddingTop: 8 }}>
            <div style={{
              ...
              fontSize: 34,
              margin: "0 auto 16px",
              boxShadow: "0 8px 40px rgba(255,140,0,0.3)",
            }}>🚀</div>
            <h1 style={{ fontSize: 24, fontWeight: 900, lineHeight: 1.3, marginBottom: 10 }}>
            ...
            <p style={{ fontSize: 14, color: "rgba(240,237,232,0.5)", lineHeight: 1.7 }}>
              {isRTL
                ? "اسألني أي سؤال في التسويق، المبيعات، أو الهوية التجارية"
                : "Ask me anything about marketing, sales, or branding"}
            </p>
          </div>
          ...
          <div className="fade-up" style={{ marginBottom: 24, animationDelay: "0.1s" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {CATEGORIES.map((cat) => (
                <div
                  key={cat.id}
                  className={`card-hover ${selectedCategory?.id === cat.id ? "cat-selected" : ""}`}
          ...
          <div className="fade-up" style={{ marginBottom: 20, animationDelay: "0.2s" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {QUICK_QUESTIONS.map((q, i) => (
                <div key={i} className="tag-pill" onClick={() => sendMessage(q)}>
                  {q}
                </div>
              ))}
            </div>
          ...
          <div className="fade-up" style={{
            ...
            gap: 10, marginBottom: 20, animationDelay: "0.3s",
          }}>
            {[
